...
def single_run(config):
    ...
    run_name = config.get("RUN_NAME") or f'{config.get("CONDITION", "C3")}_seed{config.get("SEED", 0)}'
    init_wandb_run(
        project=config.get("PROJECT", "socialjax-cleanup"),
        entity=config.get("ENTITY") or None,
        run_name=run_name,
        condition=config.get("CONDITION", "C3"),
        algo_name=config.get("ALGO_NAME", "MAPPO"),
        mechanism_class=config.get("MECHANISM_CLASS", "ClassII+III"),
        seed=config.get("SEED", 0),
        total_timesteps=int(config.get("TOTAL_TIMESTEPS", 0)),
        extra_config={"env_name": config.get("ENV_NAME", "clean_up")},
    )
    tags = ["MAPPO"] + (config.get("WANDB_TAGS") or [])
